...
int main(int argc, char *argv[])
{
    ...
    qDebug() << QObject::tr("str = ") << str;               // 结果为Hi my Qt
    str = str + "!!!";                                      // 将两个字符串组合
    qDebug() << QObject::tr("str = ") << str;               // 结果为Hi my Qt！！！
    ...
    qDebug() << QObject::tr("str2 = ") << str2;             // 结果为hi Qt！

    str = "hi,my,,Qt";
    // 从字符串中有","的地方将其分为多个子字符串，
    // QString::SkipEmptyParts表示跳过空的条目
    QStringList list = str.split("," ,Qt::SkipEmptyParts);
    qDebug() << QObject::tr("str拆分后 = ") << list;        // 结果为hi,my,Qt
    str = list.join(" ");                                  // 将各个子字符串组合为一个字符串，中间用" "隔开
    qDebug() << QObject::tr("list组合后 = ") << str;        // 结果为hi my Qt
    ...
    qDebug() << QObject::tr("'fei' pos = ") << str.indexOf("fei");        // 结果为2
    ...
    QString temp = "hello";
    if (temp > str) {                                                     // 两字符串进行比较，结果为yafeilinux
        qDebug() << temp;
    } else {
        qDebug() << str;
    }
